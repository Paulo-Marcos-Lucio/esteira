...
[![452 tests passing](https://raw.githubusercontent.com/Paulo-Marcos-Lucio/esteira/main/assets/chip-tests.svg)](https://github.com/Paulo-Marcos-Lucio/esteira/actions/workflows/ci.yml)
...
As 23 checagens do catálogo — a mesma lista que `esteira rules` imprime (um teste falha o CI se
...
| `github-env-injection` | Valor não-confiável (direto ou via `env:`) gravado em `$GITHUB_ENV` sob `pull_request_target` — quebra de linha injeta variável nova | 🔴 Crítica | A05 · CWE-74 |
...
### Precisão da injeção — o que dispara e o que não

`script-injection` decide pelo **contexto de onde o dado vem**, não pela mera presença de `${{ }}`. Além de `github.event.*` e `head_ref`, trata como **não-confiável**:

- o **`client_payload` de `repository_dispatch`** — JSON 100% controlado por quem envia o dispatch: qualquer subcaminho (`github.event.client_payload.slash_command.args.named.*`, `.args.unnamed`, …) é entrada do atacante;
- os **campos de evento de texto livre** — `release.body`/`.name`, `label.name`/`.description`, `milestone.title`/`.description` —, cujo conteúdo aceita Unicode, aspas e `$( )`.

A severidade é calibrada pelo **privilégio do gatilho**: texto livre de fork anônimo (issue / PR / comentário) permanece **Crítica**; de um ator com escrita/triagem (release, label, milestone, dispatch) é **Alta** — escalação de insider, não atacante externo anônimo.

E a ferramenta **não grita** onde não há charset de shell a injetar, evitando o falso-positivo que faria o time desligar a checagem:

- um `input` resolvido como **`type: number` ou `boolean`** em todos os blocos `on.workflow_call`/`on.workflow_dispatch` que o declaram — o GitHub coage o valor antes de ele existir (um `input` `string`/`choice`/sem `type:` continua disparando);
- `owner.login` e afins, de charset restrito `[A-Za-z0-9-]`;
- uma expressão que **inteira** avalia para booleano: os operadores `&&`/`||` do GitHub são de curto-circuito e **retornam o operando**, então `github.event.issue.title == 'x' && github.event.issue.body || 'y'` devolve o corpo da issue e **é** marcado, enquanto `contains(github.event.issue.body, 'x')` (só `true`/`false` sai) não.

Na mesma auditoria, `unpinned-container-image` passou a ancorar o achado na **chave estrutural** de onde a imagem foi lida (`container.image` / `services.<nome>.image`), nunca na primeira ocorrência textual da expressão — o que também faz o `# zizmor: ignore` / `# esteira: ignore` da linha da imagem voltar a suprimir corretamente.

---

## 🔬 O que foi medido

Números desta bateria — todos **reproduzíveis com `pytest` neste repositório** (452 testes verdes). Não são estimativa de marketing; são a régua que trava a regressão.
...
- **Corpus rotulado, versionado e público** em [`bench/`](bench/): 24 workflows positivos (**27 achados rotulados**, cobrindo as **23 de 23** regras do catálogo) e 5 negativos com **8 linhas-armadilha**. `python bench/avaliar.py` mede recall e precisão com **intervalo de Wilson** e sai com código 1 se aparecer falso-positivo ou falso-negativo; a mesma bateria roda no `pytest`, então corpus que apodrece quebra o CI. Medido em 2026-09-24: **27/27 recall, IC95% [88% ; 100%], zero falso-positivo**. **O que esse número não é:** os workflows foram escritos por quem escreveu a ferramenta; ele mede cobertura do catálogo contra casos canônicos, não acurácia contra pipelines de produção. Os limites estão listados em `bench/README.md`.
...
| Saída | Significado |
| --- | --- |
| `0` | Nenhum achado no nível de `--fail-on` ou acima **e a varredura foi completa** (inclui "caminho existe, mas não tem workflow" — o aviso vai para o stderr) |
| `1` | Achado com severidade `>= --fail-on` — **ou** a varredura foi parcial (`--only`/`--skip`) e não pode certificar ausência (ver *Cobertura*) |
...
### Cobertura — uma varredura parcial não certifica limpo
...
**A versão pública faz detecção estática completa e honesta para o que se propõe** — o catálogo de 23 checagens que trava o CI aqui, **100% offline**, com os números de campo (23 de 23 disparando na severidade fixada, zero falso-positivo no workflow endurecido). A edição **Pro** adiciona **código de confirmação ativa que não está neste repositório** — o **confirmador de ação comprometida `--online`** (consulta *read-only* a OSV/GHSA para o frescor da lista de incidentes, enviando só o **nome público** da action, nada do seu repositório) — somado ao **serviço**: adjudicar cada achado, aplicar a correção por PR e reavaliar. O diferencial não é "mais regras": é **confirma-não-explora auditável + baixo-FP com número + recorte BR/LGPD**.

| Dimensão | Ferramenta pública (você roda) | Pro · serviço (eu conduzo com você) |
| --- | --- | --- |
| **Engine** | catálogo de 23 checagens, **100% offline**, mesma saída | **as 22 checagens offline + o confirmador `--online`** (OSV/GHSA *read-only*, código Pro-exclusivo que não está no repo público) — mais o trabalho humano em cima |
...
[![E-mail](https://img.shields.io/badge/E--mail-contatopml26%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:contatopml26@gmail.com)
...
flowchart TD
    ...
    DET --> CAT["<b>checks/catalog.py</b><br/>23 checagens · severidade<br/>OWASP 2025 · CWE"]
...
**Portões, medidos agora neste repo:** 452 testes verdes (incluindo *property-based* com Hypothesis) · cobertura **97%** (gate `--cov-fail-under=93`, o medido arredondado para baixo — trava anti-regressão, não aspiração) · `mypy --strict` limpo (19 arquivos) · `ruff` lint + format limpos (58 arquivos) · CI em matriz **Python 3.10 / 3.11 / 3.12 / 3.13** (`fail-fast: false`). O comando mora no `pyproject.toml`, não no YAML: dev e CI rodam a mesma linha.
